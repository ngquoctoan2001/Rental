...
export default function InvoicesPage() {
  ...
  const [createForm, setCreateForm] = useState({ contractId: '', electricityOld: '', electricityNew: '', waterOld: '', waterNew: '', otherFees: '', discount: '', notes: '' });
  ...
  const createInvoiceMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setCreateForm({ contractId: '', electricityOld: '', electricityNew: '', waterOld: '', waterNew: '', otherFees: '', discount: '', notes: '' });
    },
    onError: (error: any) => {
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] shadow-2xl p-8 w-full max-w-md space-y-6">
            ...
            <form onSubmit={(e) => { e.preventDefault(); createInvoiceMutation.mutate({ contractId: createForm.contractId || undefined, billingMonth: selectedMonth + '-01', electricityOld: createForm.electricityOld ? Number(createForm.electricityOld) : 0, electricityNew: createForm.electricityNew ? Number(createForm.electricityNew) : 0, waterOld: createForm.waterOld ? Number(createForm.waterOld) : 0, waterNew: createForm.waterNew ? Number(createForm.waterNew) : 0, internetFee: 0, garbageFee: 0, otherFees: createForm.otherFees ? Number(createForm.otherFees) : 0, otherFeesNote: createForm.notes || undefined, discount: createForm.discount ? Number(createForm.discount) : 0, notes: createForm.notes || undefined }); }} className="space-y-4">
              ...
              <div className="p-4 bg-indigo-50/60 rounded-2xl border border-indigo-100 space-y-3">
                <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider flex items-center gap-1.5"><Zap className="w-3.5 h-3.5" /> Chỉ số điện</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-bold text-slate-600 mb-1 block">Chỉ số cũ</label>
                    <input type="number" min={0} value={createForm.electricityOld} onChange={e => setCreateForm(f => ({ ...f, electricityOld: e.target.value }))} placeholder="0" className="w-full px-3 py-2 rounded-xl border border-indigo-200 focus:ring-2 focus:ring-indigo-500/20 outline-none text-sm" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-600 mb-1 block">Chỉ số mới</label>
                    <input type="number" min={0} value={createForm.electricityNew} onChange={e => setCreateForm(f => ({ ...f, electricityNew: e.target.value }))} placeholder="0" className="w-full px-3 py-2 rounded-xl border border-indigo-200 focus:ring-2 focus:ring-indigo-500/20 outline-none text-sm font-bold text-indigo-700" />
                  </div>
                </div>
              </div>
              <div className="p-4 bg-blue-50/60 rounded-2xl border border-blue-100 space-y-3">
                <p className="text-xs font-bold text-blue-600 uppercase tracking-wider flex items-center gap-1.5"><Droplets className="w-3.5 h-3.5" /> Chỉ số nước</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-bold text-slate-600 mb-1 block">Chỉ số cũ</label>
                    <input type="number" min={0} value={createForm.waterOld} onChange={e => setCreateForm(f => ({ ...f, waterOld: e.target.value }))} placeholder="0" className="w-full px-3 py-2 rounded-xl border border-blue-200 focus:ring-2 focus:ring-blue-500/20 outline-none text-sm" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-600 mb-1 block">Chỉ số mới</label>
                    <input type="number" min={0} value={createForm.waterNew} onChange={e => setCreateForm(f => ({ ...f, waterNew: e.target.value }))} placeholder="0" className="w-full px-3 py-2 rounded-xl border border-blue-200 focus:ring-2 focus:ring-blue-500/20 outline-none text-sm font-bold text-blue-700" />
                  </div>
                </div>
              </div>
