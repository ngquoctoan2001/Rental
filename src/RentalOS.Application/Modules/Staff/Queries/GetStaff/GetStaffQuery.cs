...
public class GetStaffQueryHandler(IDbConnection dbConnection, ITenantContext tenantContext)
    : IRequestHandler<GetStaffQuery, List<StaffListItemDto>>
{
    public async Task<List<StaffListItemDto>> Handle(GetStaffQuery request, CancellationToken cancellationToken)
    {
        ...
        var staff = await dbConnection.QueryAsync<StaffListItemDto>(sql, new { tenantId = tenantContext.TenantId });
        return staff.ToList();
    }
}
