...
public class GetNotificationsQueryHandler(IDbConnection dbConnection, IUserContext userContext)
    : IRequestHandler<GetNotificationsQuery, NotificationListDto>
{
    public async Task<NotificationListDto> Handle(GetNotificationsQuery request, CancellationToken cancellationToken)
    {
        ...
        using var multi = await dbConnection.QueryMultipleAsync(sql, new { userId });
 ...
 Eskom assuming IUserContext exists for current logged in user.
